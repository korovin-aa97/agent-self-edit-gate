...
| Input | `E_INPUT_READ`, `E_INPUT_TYPE`, `E_INPUT_SIZE`, `E_SIZE`, `E_DIFF_SIZE`, `E_BINARY`, `E_ENCODING` | proposed bytes or their source violate a bound/type rule |
| Operation | `E_ACTION` | Python API requested an unsupported receipt action |
...
| Audit | `E_AUDIT_*` | journal path/link/mode, framing, sequence, chain, hash, or persistence failed |
